...
#         """, ('admin', 'admin@example.com', make_hashed_password('SPI123@_'), 'admin'))
...
#         # Create table for ranking results
#         cur.execute("""
...
#     try:
#         with get_db_cursor() as cur:
#             cur.execute("SELECT id, username, email, password, role, created_at FROM users")
#             for id, username, email, password, role, created_at in cur.fetchall():
#                 users[username] = {
#                     'password': password,
#                     'email': email,
...
# def save_users(username, password, email, role):
#     try:
...
#                 st.session_state.user_id = users[username_or_email]['id']  # Store user_id
...
#                         st.session_state.user_id = user_data['id']  # Store user_id
        ...
        if st.session_state.get('user_role') == 'admin' or not os.path.exists('users.pkl'):
            ...
            if st.button("Sign Up"):
                ...
                # Check if email already exists
                    ...
                    success = save_users(new_username, new_password, email, 'user')
                    if success:
                        st.success("Account created successfully! You can now login.")
                    else:
                        st.error("Failed to create account. Please check the logs for details.")
        else:
